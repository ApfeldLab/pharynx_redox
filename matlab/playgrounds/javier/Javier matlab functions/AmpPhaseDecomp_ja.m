function [MS_amp, MS_pha, RSQR, C] = AmpPhaseDecomp(xfd, hfd, rng)
...
%  C      ... constant C
%  Last modified Sept 19 2013 by JA from AmpPhaseDecomp
...
%yfine   = eval_fd(tfine, yfd);
    ...
    xfd_coef=getcoef(xfd);
    nfuns=size(xfd_coef,2)
    for f=1:nfuns;
        wf=[];
        wf=eval_fd(tfine,hfd(f));
        wf(wf<rng(1))=rng(1);
        wf(wf>rng(2))=rng(2);
        y1(:,f)= eval_fd(wf,xfd(f)) ;
    end
yfine= y1;
