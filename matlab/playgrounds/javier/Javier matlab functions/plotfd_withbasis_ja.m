function Hline = plotfd_withbasis_ja(fdobj, Lfdobj, matplt, href, nx)
%  plotfd_withbasis_ja   Plot a functional data object with basis.
%  Arguments:
...
%             hold on
%             adjusted_basismat = [];
%             for multiply_basismat = 1:size(coef(:,i,j),1)
%                 adjusted_basismat(:,multiply_basismat) = basismat(:,multiply_basismat) .* coef(multiply_basismat,i,j);
%             end
%             plot (xfine, adjusted_basismat , '-', 'LineWidth', 1);
%              axis square
%              set(gca,'TickDir','out')
%             hold off
%             % ------------- This added by Javier --------------
...
%     else
        ...
        for icurve = 1:ncurve
            ...
            hold on
            adjusted_basismat = [];
            for multiply_basismat = 1:size(coef(:,icurve),1)
                adjusted_basismat(:,multiply_basismat) = basismat(:,multiply_basismat) .* coef(multiply_basismat,icurve);
            end
            plot (x, adjusted_basismat , '-', 'LineWidth', 1);
             axis square
             set(gca,'TickDir','out')
