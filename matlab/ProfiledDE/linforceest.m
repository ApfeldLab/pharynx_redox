function [smooths,forces,dddy,dcdy,df] = linforceest(basis_cell,pbasis_cell,A,...
   whichindex,lambda,lambdap,f_lfd,Tcell,Ycell,wts,ident,force,force_extra)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function linforceest
%
% Estimates a forcing function to make a linear differential equation
% correspond to observed data. This is done by profiling the smooth while
% fitting the forcing functions. 
%
% INPUTS:
%
% basis_cell  - a cell array of basis objects used to represent the final
%               (forced) solutions. 
% 
...
%           empty, all weights are taken as equal. 
% 
% ident   - identification factor - penalty term, relative to lambdap, for
%           the square integral of the forcing terms. 
%
...
% First of all check all the intput is in good order.
    ...
    if nargin<13, force_extra = []; end

    if nargin<12, force = []; end
    
    if nargin<11, ident = 0; end
    
    if nargin<10, wts = ones(size(Ycell)); end

    if isempty(whichindex), whichindex = 1:length(pbasis_cell); end
...
% Now form matrices    
 ...
 % Now put it all together
    ...
    Dmat = (Zmat'*diag(wts)*Zmat + ( Dphimat - A*phimat)'*Q*(Dphimat-A*phimat));
    
    Dinv = inv(Dmat);
    E = ((Dphimat-A*phimat)'*Q*pphimat);
...
% Solve for forcing
    ...
    d = (B*diag(wts)*B'+Dpphimat'*pQ*Dpphimat+ident*pphimat'*pQ*pphimat)\...
        (B*diag(wts)*Ycell - B*diag(wts)*Zmat*Dinv*lvec);
...
%    d = res((size(Zmat,2)+1):length(res));
    ...
    if nargout > 2
        dddy = (B*diag(wts)*B'+Dpphimat'*pQ*Dpphimat+ident*pphimat'*pQ*pphimat)\...
        (B*diag(wts) - B*diag(wts)*Zmat*Dinv*Zmat'*diag(wts));
    
        dcdy = Dinv*(E*dddy + Zmat'*diag(wts));   
