...
obs_pts{2} = [];      % which observation times.
...
d2devals = eval_fdcell(tfine,DEfd,2);

X = [ddevals{1} devals{1} ones(size(devals{1}))];

startpars = (X'*X)\X'*d2devals{1};
...
[smooths,forces] = linforceest(basis_cell,basis_cell(1:2),A,1:2,10000,...
    0.001,2,Tcell,Ycell,wts,0.01,@eval_fdcell2,const_force);
