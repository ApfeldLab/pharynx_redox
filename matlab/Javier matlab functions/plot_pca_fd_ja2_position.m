function pca_plot = plot_pca_fd_ja2_position(pcastr, matplt, harm, expand, cycle, colorwrd,plotrange,save2pdf,save2pdf_name)
...
meanmat   = squeeze(eval_fd(pcastr.meanfd, x));
 %this section modifyied by Javier
meanmat = zeros(size(meanmat)); 
 %this section modifyied by Javier
dimfd     = size(fdmat);
nharm     = dimfd(2);
...
region_boundaries = [2 95; 5 9; 57 63; 29 48; 73 92];%JA
region_boundaries2 = 101- region_boundaries;%JA
plot_color={'k','b','r','m'};%JA
...
%                 num2str(percentvar), ')'])
        ...
        if matplt && length(harm) <= 4
            ...
            %end this section modifyied by Javier
        end
    end
else
