...
%     figures(i) = figure('visible','off');
    ...
    e.plotMeanStrain('reg', 'i410', strain, ax); hold on;
    plot(xs, e.reg.i410(:,i), '--','DisplayName','cos(2x)'); hold off;
    title(sprintf('Mean 410nm +/- 1.96*std of %s and worm #%d', strain, i));

    % Registered 470 & mean of Strain
    ax = subplot(3,3,8);
    strain = e.metadata.Strain{i};
    e.plotMeanStrain('reg', 'i470', strain, ax); hold on;
    plot(xs, e.reg.i470(:,i), '--','DisplayName','cos(2x)'); hold off;
    title(sprintf('Mean 470nm +/- 1.96*std of %s and worm #%d', strain, i));

    % Registered E & mean of Strain
    ax = subplot(3,3,9);
    strain = e.metadata.Strain{i};
    e.plotMeanStrain('reg', 'E', strain, ax); hold on;
    plot(xs, e.reg.E(:,i), '--'); hold off;
