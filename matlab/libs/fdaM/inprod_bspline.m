    ...
    if nrep1 == 1
        ...
        for in=1:length(index)
            if size(CoeffD,2) > 1
                ...
                polycoef1(index(in),:,:) = CoeffD(in,:)'*coef1(i,:) + ...
                            permute(polycoef1(index(in),:,:),[1,3,2]); 
            end
        end
    end
end
    ...
    if nrep2 == 1
        ...
        for in=1:length(index)
            if size(CoeffD,2) > 1
                ...
                polycoef2(index(in),:,:) = CoeffD(in,:)'*coef2(i,:) + ...
                            permute(polycoef2(index(in),:,:),[1,3,2]); 
            end
        end
    end
end
...
for j=1:ninterval     
    ...
    for i=1:D
        temp = squeeze(polyprodmat(:,:,D-i+1))./i; 
        prodmati = prodmati + power.*temp;
        power = power*delta;
    end
