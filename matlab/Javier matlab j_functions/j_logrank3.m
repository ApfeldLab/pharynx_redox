function [p,chi_square, df] = j_logrank3(strain,testtype)
%function [logrank, chi_square, df, v, sum_V] = j_logrank3(strain,testtype)
...
for test_type =1:3
    switch test_type
        ...
        case 3
            S=[];
            for m = 1:size(time_range,1)
                tmp1 = 1-sum(D,2)./ sum(A,2);
                S(m)= prod(tmp1(1:m));
                
            end
           % S'
             W = S' .* sum(A(1,:),2);
          % W = S' .* sum(A,2) ./ (1+sum(A,2));
           % size(W)
    end
    v = sum((W*ones(1,strain_number)).*(D-E)) ;
    
    Vw= (repmat(reshape(W,1,1,length(W)),[strain_number,strain_number,1]).^2).*V;
    
    sum_V = sum(Vw,3);
    df = strain_number-1;
    if df > 0 & trace(sum_V) > 0
        v_range = 1:df;
        chi_square(test_type) = v(v_range) * inv(sum(Vw(v_range,v_range,:),3)) * v(v_range)';
        p(test_type) = 1 - chi2cdf(chi_square(test_type),df);
    else
        %disp('else')
        chi_square(test_type) = 0;
        p(test_type) = 1;
    end
end
