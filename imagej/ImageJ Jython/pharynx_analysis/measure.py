...
path.append("/Users/sean/Desktop/pharynx_redox")
...
IJ.run(
    maskPA,
    "Set Scale...",
    "distance=1 known=" + str(scales[binning]) + " pixel=1 unit=μm",
)
# TODO scale?
IJ.run(
    maskPA,
    "Set Measurements...",
    "area centroid center perimeter bounding fit shape feret's median skewness kurtosis scientific redirect=None decimal=7",
)
IJ.run(
    maskPA,
    "Analyze Particles...",
    "size=0-Infinity circularity=0.00-1.00 show=Nothing stack",
)
...
for m_heading in morph_headings[1:]:  # because of formatting, the first is empty
    addValues(
        dataTable,
        m_heading,
        [morphTable.getValue(m_heading, i) for i in range(morphTable.size())],
    )
